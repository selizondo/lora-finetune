...
**What breaks:** Loading a LoRA adapter on a different base model version than it was trained on (e.g., training on `Mistral-7B-v0.1`, loading on `Mistral-7B-v0.2`). The model loads without error but produces incoherent output — no exception is raised.
